...
namespace Liaison.Import.SqlToMongo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var database = client.GetDatabase("Liaison");

            var collectionUnits = database.GetCollection<BsonDocument>("Units");


            var liaisonSql = new LiaisonSql();

            IEnumerable<IUnit> allUnits = liaisonSql.GetAllUnits();
    ...
    //            {"UnitId", unit._UnitId},
    //            { "Number",unit._Number},
    //{ "UseOrdinal", unit._UseOrdinal},
    //                { "Letter",unit._Letter },
    //                {"NickName",unit._NickName },
    //                {"LegacyMissionName", unit._LegacyMissionName },
    //                {"MissionName", unit._MissionName },
        };
    ...
    var filter = Builders<BsonDocument>.Filter.Eq("_id", 10009999);
            var documents = collectionUnits.Find(new BsonDocument()).ToList();

            var document = collectionUnits.Find(filter).First();
